...
function getStarMarkup(rating) {
  ...
  for (let i = 1; i <= 5; i += 1) {
    stars += i <= roundedRating ? '<span aria-hidden="true" class="icon-star">⭐</span>' : '<span aria-hidden="true" class="icon-star-empty">☆</span>';
  }
...
function createRecipeCard(recipe) {
  const tagsMarkup = recipe.tags
    .map((tag) => `<p class="recipe-tag">${tag}</p>`)
    .join('');

  const ingredientsMarkup = recipe.recipeIngredient
    .slice(0, 5)
  ...
  return `
    <article class="recipe-card">
      ...
      <div class="recipe-content">
        <div class="recipe-tags">${tagsMarkup}</div>
        <h2>${recipe.name}</h2>
        <p class="recipe-meta">Prep: ${recipe.prepTime} | Cook: ${recipe.cookTime} | Yield: ${recipe.recipeYield || 'Not listed'}</p>
        ${getStarMarkup(recipe.rating)}
        <p class="recipe-description">${recipe.description}</p>
        <h3 class="recipe-list-heading">Ingredients</h3>
  ...
  if (recipeList.length === 0) {
    recipesContainer.innerHTML = `
      <div class="no-results">
        <p>No recipes matched your search. Try a different word like <strong>dessert</strong>, <strong>chicken</strong>, or <strong>potatoes</strong>.</p>
      </div>
    `;
    return;
  }

  recipesContainer.innerHTML = recipeList.map(createRecipeCard).join('');
}
...
function updateRecipes(query = '') {
  ...
  if (query.trim() === '') {
    resultsSummary.textContent = 'Showing a random recipe.';
  } else if (filteredRecipes.length === 1) {
    resultsSummary.textContent = `1 recipe found for "${query.trim()}".`;
  } else {
    resultsSummary.textContent = `${filteredRecipes.length} recipes found for "${query.trim()}".`;
  }
